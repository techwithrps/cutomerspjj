...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] text-slate-900 flex flex-col font-sans selection:bg-[#0284c7] selection:text-white">
      ...
      <div className="fixed bottom-5 right-4 sm:right-6 z-50 animate-bounce">
        <a
          href={`https://wa.me/${(currentCustomer?.relationshipManager?.whatsapp || '919811044290').replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hello SPJ Relationship Manager, I am writing from ${currentCustomer.name} Client Portal. I have a query regarding our cargo invoices/containers.`)}`}
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-2 px-4 py-3 rounded-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-extrabold text-xs shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 group border-2 border-white/80"
          title="Chat with RM on WhatsApp"
        >
          ...
          <span className="inline-flex items-center px-1.5 py-0.2 rounded-full text-[8px] font-black bg-white text-[#25D366] uppercase">
            Online
          </span>
        </a>
      </div>
