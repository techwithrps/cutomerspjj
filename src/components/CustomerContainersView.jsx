      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-card flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 focus:border-[#ff6a00] rounded-xl text-xs font-semibold text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-3 focus:ring-[#ff6a00]/15 transition-all"
          />
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((item, idx) => {
            const isReefer = item.type === 'REEFER';
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2 border-b border-slate-100 pb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono font-black text-sm sm:text-base text-[#2b1f55] bg-purple-50 px-2.5 py-0.5 rounded-xl border border-purple-200">
                        {item.contNo}
                      </span>
                      <button
                ...
                {/* Live GPS / Movement Stage */}
                <div className="flex items-center justify-between text-xs pt-1">
                  <div className="flex items-center gap-1.5 text-slate-600">
                    <MapPin className="w-3.5 h-3.5 text-orange-500 shrink-0" />
                    <span className="text-[11px] font-semibold truncate">{item.liveGPS}</span>
                  </div>
                  <span className="text-[10px] font-bold text-purple-900 bg-purple-100 px-2 py-0.5 rounded-full shrink-0">
                    ETA: {item.eta}
                  </span>
                </div>
